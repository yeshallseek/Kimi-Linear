...
- Current synthetic caveat: the memory-safe vocab-16, hidden-64 palindrome LR grid did not reproduce KDA's Figure 4 advantage across two seeds; GDN had the best mean final accuracy. Extending the best-LR run to 10,000 steps narrowed the gap on one seed but still favored GDN on the two-seed mean. This result should be treated as a constrained-scale negative control until the paper-size 2-layer/2-head/head-dim-128 setup can run with enough free VRAM.
