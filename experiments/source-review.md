...
- Current synthetic status: memory-safe vocab-16, hidden-64 probes did not reproduce KDA's Figure 4 advantage and should be treated as constrained negative controls. After reclaiming the GPU, the paper-shape palindrome setup (2 layers, 2 heads, head_dim 128, hidden 256, seq_len 256, vocab 128, batch 4) showed robust KDA learning across seeds `42`, `123`, and `7` at 20,000 steps, with mean final accuracy `0.9286`. GDN was seed-sensitive: it failed on seed `42` (`0.0389`) but reached `0.9808` and `0.9906` on seeds `123` and `7`, giving mean final accuracy `0.6701`. This supports KDA robustness but does not reproduce a per-seed KDA win over GDN. Paper-shape 64-stack state tracking is the strongest learned synthetic result so far: at lr `1e-3` over seeds `42`, `123`, and `7`, KDA mean final accuracy was `0.9654` vs GDN `0.9424`, with KDA higher on two seeds and tied on one; GDN still converged faster early. MQAR remains unresolved. The original local contiguous MQAR generator was not faithful to Zoology MQAR. After patching a `--mqar-layout zoology` generator, source-style paper-shape MQAR (`vocab_size=8192`, `seq_len=256`, `num_pairs=64`) still stayed at the value-vocabulary baseline through a 20,000-step lr `1e-3` run. Tied embeddings, `std=0.02` init, higher LRs, batch 64, and an easier `num_pairs=16` slice did not fix the high-vocab setting; a tiny positive control (`vocab=256`, `seq_len=64`, `num_pairs=4`) did learn, proving the corrected harness is not dead. A follow-up `zoology_figure3` training curriculum matching the Zoology train mix let GDN solve the hard `(256,64)` eval slice by 2000 steps (`0.9838` final accuracy), but KDA stayed near chance under the same run (`0.0016`) and under a KDA-only high-LR sweep. UI should remain deferred until this MQAR gap is either fixed or explicitly accepted as out of scope.
...
- The previously occupying root-owned vLLM process was stopped with user authorization. Future runs can reclaim the GPU for this experiment when needed, recording the action. User preference as of 2026-05-24: run ML reproduction on GPU and do not add new CPU-only experiments.
...
- MQAR specifically depends on external Zoology task semantics. The local corrected generator now follows the inspected Zoology layout, but the paper-shape/high-vocab reproduction still fails under our tiny FLA wrapper and local batch/training budget.
